...
--[==[
	Name: Energize (FE Animations)
	Author: 0hc0rp
	Contributors:
	- [byteveil](https://github.com/byteveil)
]==]

local playersService = game:GetService('Players')
local player = playersService.LocalPlayer
local playerGui = type(gethui) == 'function' and gethui() or game:GetService('CoreGui')

if typeof(playerGui) ~= 'Instance' then
	--- In the case that the Player cannot use a safe UI location, the script is not executed.
	return
end
...
Credits.Text = "By 0hc0rp│FE Animations Gui"
...
--- These are global, because a single script cannot have more than 200 local variables.

animation = Instance.new('Animation')
animation.Name = 'EnergizeAnimation'
animation.AnimationId = 'rbxassetid://754656200'
animator = select(2, pcall(function()
	return player.Character:FindFirstChildWhichIsA('Humanoid'):FindFirstChildWhichIsA('Animator')
end))

if typeof(animator) == 'Instance' then
